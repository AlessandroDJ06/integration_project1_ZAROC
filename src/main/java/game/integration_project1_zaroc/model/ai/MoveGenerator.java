        ...
        for (int r1 = 0; r1 < allPegs.length; r1++) {
            for (int c1 = 0; c1 < allPegs[r1].length; c1++) {
                ...
                if (start1 == null || start1.getPawns().isEmpty() ||
                        !start1.getUpperPawn().getPawnColor().equals(activeColor)) {
                    continue;
                }
                ...
                for (Move m1 : firstMoves) {
                    ...
                    // CRITICAL FIX: Locate pegs on the COPIED board using coordinates to avoid reference errors
                    Peg tStart1 = tempGame.getBoard().getPegPosition(r1, c1);
                    Peg tDest1 = tempGame.getBoard().getPegPosition(m1.getDestinationPeg().getYPosition(), m1.getDestinationPeg().getXPosition());
                    ...
                    for (int r2 = 0; r2 < tempPegs2.length; r2++) {
                        for (int c2 = 0; c2 < tempPegs2[r2].length; c2++) {
                            ...
                            if (start2 == null || start2.getPawns().isEmpty() ||
                                    !start2.getUpperPawn().getPawnColor().equals(activeColor)) {
                                continue;
                            }
